...
@EventBusSubscriber
public class InstantXPBrewingBrewingRecipe implements IBrewingRecipe {
	@SubscribeEvent
	public static void init(RegisterBrewingRecipesEvent event) {
		event.getBuilder().addRecipe(new InstantXPBrewingBrewingRecipe());
	}
	...
	public boolean isIngredient(ItemStack ingredient) {
		return Ingredient.of(LegendaryCrateModItems.XP_ORBABERCRAFTINGRECIPIHEISTLEIDERXPORB.get()).test(ingredient);
	}

	@Override
	public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return PotionContents.createItemStack(input.getItem(), LegendaryCrateModPotions.POTION_OF_INSTANT_XP_POTION);
		}
